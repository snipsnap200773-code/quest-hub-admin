...
const AdminFacilityVisit_PC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (vData) {
        ...
        // 2. 入居者名簿と進捗の取得（targetIdForResidents を使う）
        const { data: rData } = await supabase
          .from('visit_request_residents')
          .select('*, members(name, room, floor)')
          .eq('visit_request_id', targetIdForResidents); // 💡 ここを修正
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    ...
    // 💡 完了（completed）になった瞬間だけ、その時の時刻を completed_at に記録
    if (nextStatus === 'completed') {
      updateData.completed_at = new Date().toISOString();
    } else {
      // もし間違えて完了にして「待機」に戻した場合は、日付をクリアする
